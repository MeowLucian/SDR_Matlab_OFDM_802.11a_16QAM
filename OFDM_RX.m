...
RX_Payload_1_Final=qamdemod(RX_Payload_1_no_pilot,M,0); % [1x48]
...
RX_Payload_2_Final=qamdemod(RX_Payload_2_no_pilot,M,0); % [1x48]
%% BER calculation
load('data_Payload_3'); % [1x48]
load('data_Payload_4'); % [1x48]
Error_bits=sum([abs(sign(data_Payload_3-RX_Payload_1_Final)),abs(sign(data_Payload_4-RX_Payload_2_Final))]);
BER=Error_bits/(length(data_Payload_3)+length(data_Payload_4));
%% Plot
